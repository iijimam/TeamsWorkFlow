...
インストール時の初期セキュリティを「通常」とした場合は、以下設定を事前に行ってください。
...
2. Pythonのバージョン確認と設定
    ...
    2025/1/17現在、3.13はサポートしておりません。3.12.7のインストールと必要な設定を行っていただきますよう、お願いいたします。

    設定👉[管理ポータルでの設定（Windowsでの例）](https://jp.community.intersystems.com/node/574686#comment-273661)

    ※ 2. の設定は、WindowsにインストールしたIRIS 2024.1以前では必要ありません。
...
4. SSL構成（クライアント）の作成
    ...
    プロダクションのビジネス・オペレーションで「webapi」の名称で設定しています。

    管理ポータルで作成されるか、IRISにログインし、%SYSネームスペースで以下実行することでも作成できます。

    管理ポータルの場合は、**管理ポータル > システム管理 > セキュリティ > SSL/TLS 構成** で構成名に **webapi** を設定し保存ボタンをクリックするだけです。

    ターミナルから実行する場合は、以下の通りです。
